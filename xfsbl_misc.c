 ...
 *
 * @return	0 if both strings are same, -1/1 if first non matching character
 *has lower/greater value in Str1Ptr
 *
 ******************************************************************************/
static s32 XFsbl_Strcmp(const char* Str1Ptr, const char* Str2Ptr) {
  s32 retVal;
...
void XFsbl_MakeSdFileName(char* XFsbl_SdEmmcFileName, u32 MultibootReg,
  ...
  if (0x0U == MultiBootNum) {
    ...
    for (Index = FileNameLen - 1U;
         Index >= (FileNameLen - XFSBL_NUM_DIGITS_IN_FILE_NAME); Index--) {
      Value = MultiBootNum % 10U;
      MultiBootNum = MultiBootNum / 10U;
      XFsbl_SdEmmcFileName[Index] += (char)(Value & 0xFF);
      if (MultiBootNum == 0U) {
        break;
      }
    }
  ...
  XFsbl_Printf(DEBUG_INFO, "File name is %s\r\n", XFsbl_SdEmmcFileName);
}
...
void XFsbl_RegisterHandlers(void) {
  ...
  Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_SYNC_INT,
                               (Xil_ExceptionHandler)XFsbl_UndefHandler,
                               (void*)0);
  Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_IRQ_INT,
                               (Xil_ExceptionHandler)XFsbl_IrqHandler,
                               (void*)0);
  Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_FIQ_INT,
                               (Xil_ExceptionHandler)XFsbl_FiqHandler,
                               (void*)0);
  Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_SERROR_ABORT_INT,
                               (Xil_ExceptionHandler)XFsbl_DataAbortHandler,
                               (void*)0);
#else
  ...
  Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_SWI_INT,
                               (Xil_ExceptionHandler)XFsbl_SvcHandler,
                               (void*)0);
  Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_PREFETCH_ABORT_INT,
                               (Xil_ExceptionHandler)XFsbl_PreFetchAbortHandler,
                               (void*)0);
 ...
 * @return	None.
 *
 * @note		The MMU and D-cache need not be disabled before changing
 *an translation table attribute.
 *
 ******************************************************************************/
...
const char* XFsbl_GetProcEng(void) {
  ...
  if ((XFsbl_In32(EFUSE_IPDISABLE) & EFUSE_IPDISABLE_CG_MASK) ==
    ...
    return "CG";
  } else if ((DevSvdId == 0x20U) || (DevSvdId == 0x21U) ||
             (DevSvdId == 0x30U)) {
 ...
 *		Error on failure.
 *
 * @note		Cache invalidation and flushing should be taken care by
 *user Before calling this API ADMA also should be configured to simple DMA.
 *
 ******************************************************************************/
u32 XFsbl_AdmaCopy(void* DestPtr, void* SrcPtr, u32 Size) {
  ...
  XFsbl_Out32(
      ADMA_CH0_ZDMA_CH_SRC_DSCR_WORD1,
      (((u64)SrcAddr >> 32U) & ADMA_CH0_ZDMA_CH_DST_DSCR_WORD1_MSB_MASK));

  /* Write Destination Address */
  XFsbl_Out32(ADMA_CH0_ZDMA_CH_DST_DSCR_WORD0,
              (u32)(DstAddr & ADMA_CH0_ZDMA_CH_DST_DSCR_WORD0_LSB_MASK));
  XFsbl_Out32(
      ADMA_CH0_ZDMA_CH_DST_DSCR_WORD1,
      (u32)((DstAddr >> 32U) & ADMA_CH0_ZDMA_CH_DST_DSCR_WORD1_MSB_MASK));
...
/**
 * *
 * * This macro polls an address periodically until a condition is met or till
 * the
 * * timeout occurs.
 * * The minimum timeout for calling this macro is 100us. If the timeout is less
 * * than 100us, it still waits for 100us. Also the unit for the timeout is
 * 100us.
